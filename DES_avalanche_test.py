...
class DES(): 
	...
	def __init__(self, text, key, nRound = 16, blockSize = 64, seed = None, dif = 0):
		...
		self.org = self.text
		self.orgD = dif
		while findDifference(self.org,self.text)!=dif:
			inv = random.randint(0,len(self.text)-1)
			newString = self.text[:inv]
			newString += INV(self.text[inv])
			newString += self.text[inv+1:]
			self.text = newString

		# print(self.org)
		# print(self.text)
		# print(findDifference(self.org,self.text))
		assert(findDifference(self.org,self.text)==self.orgD)
...
def print3D():
	key = "umangjai"
	text= "Nothing_is_absolutefjksdhfkjladshfslglagfhadfgllf"

	fig = plt.figure()
	ax = plt.axes(projection="3d")

	Xtemp = np.linspace(2, 16, 15).astype('int')
	Ytemp = np.linspace(1, 30, 30).astype('int')
	...
	for x in Xtemp:
		for y in Ytemp:
			d = DES(text, key, x, 64, _seed, 0)
			originalEncryptedText  = d.run("encrypt")

			dtemp = DES(text, key, x, 64, _seed, y)
			encryptedText  = dtemp.run("encrypt")
			z = int(findDifference(string_to_binary(originalEncryptedText), string_to_binary(encryptedText)))
			# print("{} {} {} ".format(x,y,z))
			Z.append(z)
			X.append(x)
			Y.append(y)
	...
	ax.set_ylabel('Difference in plain text')
	ax.set_zlabel('Difference in cipher text')
	plt.title("avalanche graph")
	plt.show()


def print2D(bitDiffs):
	key = "umangjai"
	text= "Nothing_is_absolute"

	Xtemp = np.linspace(1, 32, 32).astype('int')
	_seed = 256
	for bitDiff in bitDiffs:
		...
		for x in Xtemp:
			...
			d = DES(text, key, 64, 64, _seed, 0)
			originalEncryptedText  = d.run("encrypt")

			dtemp = DES(text, key, x, 64, _seed, bitDiff)
			encryptedText  = dtemp.run("encrypt")

			y = int(findDifference(string_to_binary(originalEncryptedText), string_to_binary(encryptedText)))
			X.append(x)
			Y.append(y)

		plt.plot(X, Y, label = "differnce in plaintext is {}".format(bitDiff))
		# print(max(Y))

	# naming the x axis
	plt.xlabel("Rounds")
	# naming the y axis
	plt.ylabel("Difference in cipher text")
...
def print2D2(nRounds):
	key = "umangjai"
	text= "Nothing_is_absolute"

	Ytemp = np.linspace(1, 32, 32).astype('int')
	_seed = 256
	for x in nRounds:
		...
		plt.plot(Y, Z, label = "No of rounds in DES is {}".format(x))

	# naming the x axis
	plt.xlabel("differnce in plaintext")
	# naming the y axis
	plt.ylabel("Difference in cipher text")
	# giving a title to my graph
	plt.title("avalanche graph")
